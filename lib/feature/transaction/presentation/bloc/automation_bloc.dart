...
class AutomationBloc extends HydratedBloc<AutomationEvent, AutomationState> {
  ...
  AutomationBloc(this.createTransactionUseCase) : super(AutomationInitial()) {
    ...
    on<CreateManualTransaction>(_onCreateManual);
  }
  Future<void> _onCreateManual(
    CreateManualTransaction event,
    ...
    try {
      ...
      emit(TransactionSuccess("Transaction saved successfully!"));

      emit(AutomationLoaded(pendingTransactions: state.pendingTransactions));
    } catch (e) {
      if (kDebugMode) {
  ...
  void _onDetected(TransactionDetected event, Emitter<AutomationState> emit) {
    final updatedList = List<DetectedTransaction>.from(
  ...
  Future<void> _onCategorize(
    CategorizeTransaction event,
    Emitter<AutomationState> emit,
  ) async {
    try {
      await createTransactionUseCase(
  ...
  AutomationState? fromJson(Map<String, dynamic> json) {
    try {
      ...
      final pending = pendingJson
          .map(
            (item) => DetectedTransaction.fromMap(item as Map<String, dynamic>),
          )
          .toList();
